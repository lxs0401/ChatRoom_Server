...
//	private UserModel model;
    ...
    private void processLogin() throws Exception {
        ...
        if (msg.getType() == 0x01) {
            ...
            UserInfo newUser = model.createUser(mr.getPwd(), mr.getNikeName(), 1,mr.getIdentityKey(),mr.getSignedPreKey(),mr.getSignedPreKeySig(),mr.getReistrationId(),mr.getPreKeys());
...
//			System.out.println("From "+from+" To "+to+" Text "+msgText);
        ...
        {
            System.out.println("Get keys request");
            MsgSignalKeys msk = (MsgSignalKeys) msg;
            DBConnection conn = DBConnection.getInstance();

            MsgSignalKeysResp keys = conn.getFriendsKeys(msk.getFriend_id());
            keys.setDest(msg.getSrc());
            keys.setSrc(Figures.ServerJK);
            keys.send(ous);
            conn.close();
        }
//        else if(msg.getType() == 0x07)
//        {
//            MsgCanRequest mcq = (MsgCanRequest) msg;
//            DBConnection conn = DBConnection.getInstance();
//
//            MsgCanRequestResp mcqr = conn.getFriendsKeys();
//            mcqr.setDest(msg.getSrc());
//            mcqr.setSrc(Figures.ServerJK);
//            mcqr.send(ous);
//            conn.close();
//        }
    }
    ...
    public void sendMsg(int from, byte[] msg) throws IOException {
